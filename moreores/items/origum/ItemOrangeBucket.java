...
public class ItemOrangeBucket extends Item
{
	/** field for checking if the bucket has been filled. */
	private int isFull;

	public ItemOrangeBucket(int par1, int par2)
	{
		super(par1);
		this.maxStackSize = 1;
		this.isFull = par2;
			...
			if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
			{
				...
				if (this.isFull == 0)
				{
					...
					if (par2World.getBlockMaterial(i, j, k) == Material.water && par2World.getBlockMetadata(i, j, k) == 0)
					{
						...
						if (--par1ItemStack.stackSize <= 0)
						{
							return new ItemStack(MoreOres.orangeBucketWater);
						}

						if (!par3EntityPlayer.inventory.addItemStackToInventory(new ItemStack(MoreOres.orangeBucketWater)))
						{
							par3EntityPlayer.dropPlayerItem(new ItemStack(MoreOres.orangeBucketWater.itemID, 1, 0));
						}
				...
				else
				{
					if (this.isFull < 0)
					{
						return new ItemStack(MoreOres.orangeBucketEmpty);
					}
	...
	public void registerIcons(IconRegister iconRegister){
		if(this.isFull == Block.waterMoving.blockID){
			itemIcon = iconRegister.registerIcon("moreores:orangeBucketWater");
		}
		else {
			itemIcon = iconRegister.registerIcon("moreores:orangeBucketEmpty");
		}
	}
}
