...
import moreores.worldgen.WorldGeneratorGreenTrees;
...
public class BlockGreenSapling extends BlockFlower
{

	public static final String[] WOOD_TYPES = new String[] {"green"};
	public BlockGreenSapling(int i, int j)
	{
		super(i);
		float var3 = 0.4F;
			...
			for (i1 = 0; i1 >= -1; --i1)
			{
				for (j1 = 0; j1 >= -1; --j1)
				{
					if (this.isSameSapling(par1World, par2 + i1, par3, par4 + j1, 3) && this.isSameSapling(par1World, par2 + i1 + 1, par3, par4 + j1, 3) && this.isSameSapling(par1World, par2 + i1, par3, par4 + j1 + 1, 3) && this.isSameSapling(par1World, par2 + i1 + 1, par3, par4 + j1 + 1, 3))
					{
						...
						object = new WorldGeneratorGreenTrees(true);
						flag = true;
						break;
					}
				}
	...
	public void registerIcons(IconRegister reg){
		this.blockIcon = reg.registerIcon("moreores:greenSapling");
	}  
}