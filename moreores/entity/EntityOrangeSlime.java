...
public class EntityOrangeSlime extends EntityLiving implements IMob
{
    ...
    public EntityOrangeSlime(World par1World)
    {
        super(par1World);
        ...
        if (this.onGround && !flag)
        {
            ...
            for (int j = 0; j < i * 8; ++j)
            {
                ...
                if(this.worldObj.isRemote){
                	MoreOresParticleEffects.spawnParticle("orangeSlime", this.posX + (double)f2, this.boundingBox.minY, this.posZ + (double)f3, 0.0D, 0.0D, 0.0D);
                }
            }
    ...
    protected EntityOrangeSlime createInstance()
    {
        return new EntityOrangeSlime(this.worldObj);
    }
    ...
    public void setDead()
    {
        ...
        if (!this.worldObj.isRemote && i > 1 && this.func_110143_aJ() <= 0.0F)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                ...
                EntityOrangeSlime entityslime = this.createInstance();
	...
	{
		return StatCollector.translateToLocal("\u00A76Orange Slime");
	}
}
