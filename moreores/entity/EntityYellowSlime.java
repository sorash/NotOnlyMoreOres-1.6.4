...
public class EntityYellowSlime extends EntityLiving implements IMob
{
    ...
    public EntityYellowSlime(World par1World)
    {
        super(par1World);
        ...
        if (this.onGround && !flag)
        {
            ...
            for (int j = 0; j < i * 8; ++j)
            {
                ...
                if(this.worldObj.isRemote){
                	MoreOresParticleEffects.spawnParticle("yellowSlime", this.posX + (double)f2, this.boundingBox.minY, this.posZ + (double)f3, 0.0D, 0.0D, 0.0D);
                }
            }
    ...
    {
        return this.rand.nextInt(20) + 3;
    }

    protected EntityYellowSlime createInstance()
    {
        return new EntityYellowSlime(this.worldObj);
    }
    ...
    public void setDead()
    {
        ...
        if (!this.worldObj.isRemote && i > 1 && this.func_110143_aJ() <= 0.0F)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                ...
                EntityYellowSlime entityslime = this.createInstance();
    ...
    {
        return this.getSlimeSize() == 1 ? MoreOres.yellowJelly.itemID : 0;
    }
	...
	{
		return StatCollector.translateToLocal("\u00A7eYellow Slime");
	}
}
